...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/05/18 13:45:44 by clim              #+#    #+#             */
/*   Updated: 2021/05/18 13:45:46 by clim             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		mlx_draw(t_scene *scene, t_camera *camera, t_vec *cn_lc_ve_ho)
{
	...
	while (++j < scene->viewport.height)
	{
		...
		{
			u_v[0] = (scene->viewport.height - (double)j) / \
							(scene->viewport.height);
			u_v[1] = (scene->viewport.width - (double)i) / \
							(scene->viewport.width);
			a_b[0] = camera->orig;
			a_b[1] = vplus(cn_lc_ve_ho[1], vplus(vmult_(cn_lc_ve_ho[3], \
							u_v[1]), vmult_(cn_lc_ve_ho[2], u_v[0])));
			pixel_color = ray_color((new_ray(a_b[0], a_b[1])), scene);
...
void		start_mlx(t_scene *scene)
{
	...
	cn_lc_ve_ho[3] = vcross(vec(0, 1, 0), vunit(cam->vec));
