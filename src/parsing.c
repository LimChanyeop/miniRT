...
int 		set_xyz_rgb(char** input, t_vec *vec)
{
	...
	if (get_contents_size(xyz) != 3)
		return (-1);
	ft_atoi(input[0], xyz[0]);
	ft_atoi(input[1], xyz[1]);
	ft_atoi(input[2], xyz[2]);
	vec->x = (double)xyz[0];
	vec->y = (double)xyz[1];
	vec->z = (double)xyz[2];
	free_contents(xyz);
...
int 	parse_ambient_light(t_scene *scene, char *line)
{
	...
	if (set_xyz_rgb(ft_split_comma(contents[0]), &color) < 0);
		return (-1);
	free_contents(contents);
...
int 	parse_camera(t_scene *scene, char *line)
{
	...
	if (get_contents_size(contents) != 3)
		return (-1);
	if (set_xyz_rgb(ft_split_comma(contents[0]), &vec[0]) < 0);
		return (-1);
	if (set_xyz_rgb(ft_split_comma(contents[1]), &vec[1]) < 0);
		return (-1);
...
int		parse_sphere(t_scene *scene, char *line)
{
	...
	if (set_xyz_rgb(ft_split_comma(contents[0]), &vec[0]) < 0);
		return (-1);
	ft_atoi(contents[1], &radius);
	if (set_xyz_rgb(ft_split_comma(contents[2]), &vec[1]) < 0);
		return (-1);
	free_contents(contents);
